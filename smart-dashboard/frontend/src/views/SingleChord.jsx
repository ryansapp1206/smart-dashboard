export default function SingleChord({ chordName, chordData }) {
  ...
  return (
    <div key={chordName} className="fixed inset-0 box-border bg-zinc-950 py-12 px-32 flex flex-col items-center justify-center animate-fade-in overflow-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-zinc-900 to-zinc-950">
      <h1 className="text-[6rem] font-black text-zinc-100 mb-12 tracking-widest uppercase drop-shadow-md">{chordName}</h1>
      <svg className="h-[75vh] w-auto drop-shadow-2xl" viewBox="0 0 600 1000">
        ...
        {fretArray.map(fret => <line key={`fret-${fret}`} x1="50" y1={fret * 200 + 120} x2="550" y2={fret * 200 + 120} stroke="#3f3f46" strokeWidth="6" />)}
        {stringArray.map((string, idx) => <line key={`string-${string}`} x1={string * 100 + 50} y1="120" x2={string * 100 + 50} y2="920" stroke="#71717a" strokeWidth={6 - idx * 0.6} />)}
