export default function DashboardHeader({ time, today, apiLatency, requestCount, weather }) {
  return (
    <div className="flex justify-between items-start shrink-0 mb-10 w-full">
      ...
      {/* Left section: Current date and large active time display */}
      ...
      {/* Center section: System telemetry bar tracking IP, ping latency, request count, and Vosk status */}
      <div className="flex-1 flex justify-center items-start pt-12">
        <div className="flex items-center gap-8 font-mono text-[1.8rem] text-zinc-500 bg-zinc-900/40 border border-zinc-800/80 px-10 py-8 rounded-full backdrop-blur-sm">
          ...
          {/* Real-time backend API round-trip ping */}
          ...
          {/* Total local state poll counter */}
      ...
      {/* Right section: Open-Meteo weather widget and dynamic condition icons */}
      <div className="flex items-center gap-8 shrink-0 mt-8">
        {weather && (
          <>
            {/* Clear sky icon (WMO Code 0) */}
            ...
            {/* Cloudy / Foggy conditions icon (WMO Codes 1-3, 45, 48) */}
            ...
            {/* Snow conditions icon (WMO Codes 71-77, 85, 86) */}
            ...
            {/* Rain / Thunderstorm conditions icon (WMO Codes 51-67, 80-82, 95-99) */}
